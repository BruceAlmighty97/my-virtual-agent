...
export class DocumentService implements OnModuleInit {
    ...
    async onModuleInit() {
        try {
            const data = await this._s3Client.send(new ListObjectsV2Command({ Bucket: this._s3BucketName }));
    ...
    private async downloadAndExtractText(fileKey: string): Promise<string> {
        try {
            ...
            await pipeline(response.Body as stream.Readable, createWriteStream(tempFilePath));
